  ...
  return (
    <Card key={idx} className={styles.card} sx={{ boxShadow: "unset" }}>
      ...
      <div className={styles.card__button_group}>
        <div className={styles.buttons__row}>
          <ThemeProvider theme={themeCardBtnSize}>
            {types.map((item, idx) => (
              <CardBtnSize
                onClick={() => setSelectWidth(item)}
                key={idx}
                className={`${selectWidth === item ? styles._active : null}`}
                variant="contained"
                sx={{ width: "100%" }}
              >
                {widthPizza[item]}
        ...
        <div className={`${styles.buttons__row} ${styles.buttons__size}`}>
          <ThemeProvider theme={themeCardBtnSize}>
            {sizes.map((item, idx) => (
              <CardBtnSize
                onClick={() => setSelectSize(idx)}
                key={idx}
                className={`${selectSize === idx ? styles._active : null}`}
                variant="contained"
                sx={{ width: "100%" }}
              >
                {item} см
      ...
      <div className={styles.card__button_buy}>
        ...
        <ThemeProvider
          theme={count > 0 ? themeCardBtnBuyToCount : themeCardBtnBuy}
        >
          <CardBtnBuy
